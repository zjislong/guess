%%%-------------------------------------------------------------------
%% @doc guess top level supervisor.
%% @end
%%%-------------------------------------------------------------------
...
init([]) ->
    {_, ServerType} = application:get_env(guess, server_type),
    {_, ServerVar} = application:get_env(guess, server_var),
    Child = case ServerType of
        main ->
            [
                %%主进程
                #{id => guess, start => {guess_srv, start_link, [main, ServerVar]}}
            ];
        player ->
            [
                %%主进程
                #{id => guess, start => {guess_srv, start_link, [player, ServerVar]}},
                %%定时器
        ...
        game ->
            [
                %%主进程
                #{id => guess, start => {guess_srv, start_link, [game, ServerVar]}},
                %%排行榜
